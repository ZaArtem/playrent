...
export default async function Page({
	...
	const result = await db.query(`SELECT * FROM ONLY client WHERE id=${params.id.replace("%3A", ":")} FETCH actual_address,registration_address`);
	console.log("test ", result[0].result);
	return <Client client={result[0].result} submit={submit}></Client>;
}